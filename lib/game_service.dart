...
// Serviço de jogo que gerencia a lógica do jogo e interage com o Firestore
class GameService with ChangeNotifier {
  ...
  List<Question> _generateLevel1Questions() {
    ...
    // Adiciona a pergunta sobre Network ID
    ...
    // Adiciona a pergunta sobre Broadcast
    questions.add(Question(
  ...
  String _calculateNetworkId(String ip, int mask) {
    List<int> octets = ip.split('.').map(int.parse).toList(); // Converte o IP em uma lista de octetos
  ...
  List<String> _generateOptions(String correctAnswer) {
    ...
    return options.toList()..shuffle(); // Converte o conjunto em uma lista e embaralha
  }

  // Método para salvar a pontuação do usuário no Firestore
  Future<void> saveScore(String uid, int score) async {
    DocumentReference userDoc = _firestore.collection('users').doc(uid); // Referência ao documento do usuário
    DocumentSnapshot userSnapshot = await userDoc.get(); // Obtém o documento do usuário

    if (userSnapshot.exists) {
      int currentScore = userSnapshot['score']; // Obtém a pontuação atual do usuário
  ...
  // Método para obter a pontuação do usuário do Firestore
  Future<int> getUserScore(String uid) async {
    DocumentSnapshot userSnapshot = await _firestore.collection('users').doc(uid).get(); // Obtém o documento do usuário
    if (userSnapshot.exists) {
      return userSnapshot['score']; // Retorna a pontuação do usuário
    }
    return 0; // Retorna 0 se o documento não existir
  }

  // Método para obter os 5 melhores scores dos usuários
